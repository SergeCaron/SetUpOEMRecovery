...
if ( Test-Path -Path "$USBRecovery\Drivers" -PathType Container ) {
...
else  {	New-Item "$USBRecovery\Drivers" -ItemType Directory
		...
		ForEach ( $Ressources in $LanguagePacks )
		 { if ( Test-Path -Path "$USBRecovery\Drivers\$Ressources" -PathType Container )
		   ...
		   Try { Copy-Item -Path "$Env:WinDir\System32\$Ressources\PnPUtil.exe.mui" `
...
$LargeDrivers = $Drivers | Where {  $(Get-ChildItem $_.FullName -Recurse -File | Measure-Object -Property Length -Sum | Select-Object -ExpandProperty Sum) -ge 40mb }
...
$Drivers | ft Name, LastWriteTime -AutoSize
